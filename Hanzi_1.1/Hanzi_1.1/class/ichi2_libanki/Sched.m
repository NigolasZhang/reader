...
- (void)_updateCutoff
{
//MMM        int oldToday = mToday;
          NSInteger oldToday = _mToday;
...
//        mToday = (int) ((Utils.now() - mCol.getCrt()) / 86400); // 86400=60*60*24, one day;
    self.mToday = ([[Utils now] intValue] - [self.mCol getCrt]) / 86400;
    
//        // end of day cutoff mDayCutoff是今天结束的那一刻，给个界定
//        mDayCutoff = mCol.getCrt() + ((mToday + 1) * 86400);  // 计算截至日期的时间，精确到秒
    self.mDayCutoff = [self.mCol getCrt] + ((self.mToday + 1) * 86400);
...
//        }
    if (oldToday != self.mToday) {
        [self.mCol logWithargs:[NSNumber numberWithInt:self.mToday], [NSNumber numberWithInt:self.mDayCutoff]];
    }
...
//        }
    ...
    for (NSMutableDictionary *deck in [[self.mCol getDecks] all]) {
        [self updateWithg:deck];
    }
...
//init-names-did-revCount-lrnCount-newCount这个方法找不到，您自己匹配吧！

//init-name-did-revCount-lrnCount-newCount这个方法找不到，您自己匹配吧！
//
//        /**
...
//DeckDueTreeNode-did这个方法找不到，您自己匹配吧！
