...
//        }
    _mChanged = FALSE;
    _mModels = [NSMutableDictionary dictionary];
    NSMutableDictionary *modelarray = [NSJSONSerialization JSONObjectWithData:[json dataUsingEncoding:NSUTF8StringEncoding]options:NSJSONReadingMutableLeaves error:nil];
    NSArray *ids = [modelarray allKeys];
    
    if (ids != nil) {
        
        for (int i = 0; i<ids.count; i++) {
            NSString *xid = ids[i];
            NSMutableArray *o = [modelarray objectForKey:xid];
            [_mModels setObject:xid forKey:o];
        }
    }
...
//init-_idx-_oldidx这个方法找不到，您自己匹配吧！
//
//
//        @Override
//        /**
...
//transform-fields这个方法找不到，您自己匹配吧！
//
//
